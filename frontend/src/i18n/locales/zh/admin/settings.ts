export default {
    settings: {
      ...
      openaiFastPolicy: {
        ...
        scopeBedrock: '仅 Bedrock 账号',
        apiKeys: '指定 API Key',
        apiKeysHint: '输入 Key 名称搜索。留空表示不按网关 API Key 限制；不能与指定用户同时使用。',
        apiKeySearchPlaceholder: '输入 API Key 名称搜索',
        apiKeySearchEmpty: '未找到匹配 API Key',
        ...
        removeUser: '移除用户',
        includeMissingTier: '同时匹配未传 service_tier 的请求',
        errorMessage: '错误消息',
        errorMessagePlaceholder: '拦截时返回的自定义错误消息',
        errorMessageHint: '留空则使用默认错误消息。',
        modelWhitelist: '目标模型',
